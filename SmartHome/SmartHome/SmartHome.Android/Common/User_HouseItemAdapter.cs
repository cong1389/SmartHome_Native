...
{
    class User_HouseItemAdapter : BaseAdapter<House>
    {
        ...
        public User_HouseItemAdapter(Activity currentContext, List<House> lstHouse,string roleName)
        {
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (convertView == null)
                convertView = currentContext.LayoutInflater.Inflate(Resource.Layout.User_HouseItem, null);

            TextView txtName = convertView.FindViewById<TextView>(Resource.Id.txtName);
            txtName.Text =string.Format("{0} ({1})", item.name,roleName) ;
